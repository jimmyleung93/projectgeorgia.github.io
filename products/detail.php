...
<div id="main">
		...
		if (database[i].ID == id) {
			for (i2 = 0; i2 < Object.keys(database[i].CPU).length; i2++) {
				if (database[i].Price[i2] == "N/A") {
					...
					if (!seleted) {
						document.getElementById('productbtngroup').innerHTML += '<button class="productbtn seleted" id="' + (i2 + 1) + '" onclick="javascript:SeletedSpec(' + (i2 + 1) + ');">' + database[i].Short[i2] + '</button>';
						seleted = true;
						seletednum = i2;
					} else {
			...
			document.getElementById("Battery").textContent = database[i].Battery;
	...
	while (x != null) {
			if (i != num && x.className != "productbtn disabled") {
				...
				x.className = "productbtn seleted";
			}
		i++;
