...
<div id="main">
	...
	<div class="productlabel" id="productlabel">
	...
	for (idpos = 0; idpos < numofid; idpos++) {
		for (let i in database) {
			if (database[i].ID == IDArray[idpos]) {
				document.getElementById('productlabel').innerHTML += '<span class="dot' + (idpos + 1) + '"></span><span>' + database[i].Name + '<br><small>(' + database[i].Short[IDVersionArray[idpos]] + ')</small></span>';
				CompareArray.push(database[i].CPU1[IDVersionArray[idpos]]);
			}
		}
	}
	
	if (numofid < 10) {document.getElementById('productlabel').innerHTML += '<span class="dot0"></span><button class="addproductbtn" onclick="AddProductDiv()">Add another product</button>'};
